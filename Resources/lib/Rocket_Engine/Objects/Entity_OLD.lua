...
function Entity.new(collider_position_x,collider_position_y,collider_width,collider_height)
    ...
    e.physics_data.in_air=false -- is the entity in the air?
    e.bounciness=.7
...
function Entity:update(dt,collision_override)
    ...
    if(self.physics_data.in_air and (self.bounces_left > 0 and self.velocity.y ~= 0)) then
        ...
        if ((self.local_position + bounce_delta).y <= 0) then
            ...
            self.velocity.z = self.velocity.z * self.bounciness;
        ...
        if(not self.going_into_cannon and self.local_position~=vector.new(0,0)) then
